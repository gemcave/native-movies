...
import { SPIDERMAN_DETAILS } from '../../routes';
import { WHITE, BLUE } from '../../constants';

const moviesUrl = 'http://api.tvmaze.com/search/shows?q=spider-man';

const HomeScreen: props => React$Node = props => {
  const [title, _] = useState('Spiderman');
        ...
        <View style={container}>
          {movies.map(movie => (
            <ImageCard
              ...
              data={movie.show}
              onPress={() =>
                navigation.navigate(SPIDERMAN_DETAILS, {
                  show: movie.show,
                  onGoBack: onGoBack,
                })
              }
